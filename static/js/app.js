...
function setPage(page){const allowed=rolePages();if(!allowed.some(p=>p.id===page)){page='dashboard';}state.page=page;state.filters={};state.current_supplier_id=null;state.current_po_id=null;render();document.querySelector('.sidebar')?.classList.remove('open');document.getElementById('sidebarOverlay')?.classList.remove('open');}
...
async function handleLogin(e){e.preventDefault();const email=document.getElementById('loginEmail').value;const password=document.getElementById('loginPassword').value;const btn=document.getElementById('loginBtn');if(btn){btn.textContent='Signing in...';btn.disabled=true;}const r=await api('/api/auth/login',{method:'POST',body:JSON.stringify({email,password})});if(r.ok){localStorage.setItem('token',r.data.access_token);const me=await api('/api/auth/me');state.user=me.ok?me.data:{email,role:'admin'};toast('Login successful','success');state.page='dashboard';render();loadSettings();loadDashboard();}else{const err=document.getElementById('loginError');if(err)err.textContent=r.data.detail||'Invalid credentials';}if(btn){btn.textContent='Sign In';btn.disabled=false;}}
function rolePages(){const role=state.user?.role||'reception';const all=[{id:'dashboard',icon:'dashboard',label:'Dashboard',roles:['admin','reception','technician','warehouse']},{id:'repairs',icon:'repairs',label:'Repairs',roles:['admin','reception','technician']},{id:'customers',icon:'customers',label:'Customers',roles:['admin','reception']},{id:'inventory',icon:'inventory',label:'Inventory',roles:['admin','warehouse','technician']},{id:'services',icon:'services',label:'Services',roles:['admin','reception']},{id:'payments',icon:'payments',label:'Payments',roles:['admin','reception']},{id:'expenses',icon:'expenses',label:'Expenses',roles:['admin','reception']},{id:'daily',icon:'daily',label:'Daily Sales',roles:['admin','reception']},{id:'suppliers',icon:'suppliers',label:'Suppliers',roles:['admin','warehouse']},{id:'purchase_orders',icon:'purchase_orders',label:'Purchase Orders',roles:['admin','warehouse']},{id:'reports',icon:'reports',label:'Reports',roles:['admin','reception','technician','warehouse']},{id:'shops',icon:'shops',label:'Shops',roles:['admin','warehouse']},{id:'collections',icon:'collections',label:'Collections',roles:['admin','warehouse']},{id:'logs',icon:'reports',label:'Activity Logs',roles:['admin']},{id:'staff',icon:'staff',label:'Staff',roles:['admin']},{id:'settings',icon:'settings',label:'Settings',roles:['admin']}];return all.filter(p=>p.roles.includes(role));}
function renderSidebar(){const pages=rolePages();return`<div class="sidebar" id="sidebar"><div class="sidebar-header"><h2>${esc(state.settings.shop_name||'Shop Management')}</h2><p>${esc(state.user?.role||'')}</p></div><nav class="sidebar-nav">${pages.map(n=>`<div class="nav-item${state.page===n.id?' active':''}" onclick="setPage('${n.id}')">${ICONS[n.icon]}${n.label}</div>`).join('')}</nav><div class="sidebar-footer"><div class="nav-item" onclick="handleLogout()">${ICONS.logout}Logout</div></div></div>`;}
